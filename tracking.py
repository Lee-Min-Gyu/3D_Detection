...
class Opponent_state:
    ...
    def predict (self):
        if self.useTargetVel: ##시야에서 사라진 경우 tartget velocity를 따른다고 가정
...
class ObstacleSD:
    ...
    def update_mean(self,track_length): ##s값과 d 값의 측정 평균을 구하고
        if (self.nb_meas == 0):
    ...
    def isStatic(self,track_length): # total_count와 static_count의 비율을 구해서 절반 이상이 정적이라하면 정적 장애물인 것이여
...
class StaticDynamic(Node):
    ...
    def get_closest_pos(self, max_dist: float, obstacle_position: Tuple[float, float], meas_obstacles_copy: List[Obstacle]) -> Tuple[List[Obstacle], List[float]]:
        ...
        for meas_obstacle in meas_obstacles_copy:
            ...
            ##측정한 장애물들 중에 obstacle_position과 특정 거리(max_dist)보다 가까운 거리에 있는 장애물들을 potential_obs로 따로 분류한다. 거리값도 같이 저장
            if (dist < max_dist):
    ...
    def verify_position(self, obstacle: ObstacleSD, meas_obstacles_copy: List[Obstacle]) -> Optional[Obstacle]:
        ...
        if obstacle.staticFlag == False: #추적 중인 장애물이 동적
            obstacle_position = (
    ...
    def check_in_front(self, tracked_obstacle: ObstacleSD, car_s: float) -> bool:
        ...
        return 0 < obj_dist_in_front < self.dist_infront # 정한 거리보다 작으면 앞에 있다 판단
